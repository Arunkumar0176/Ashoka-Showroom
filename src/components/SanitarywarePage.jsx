...
function ShareOnCard({ item, categoryName }) {
  const [copied, setCopied] = useState(false);
  const handleShare = async (e) => {
    e.stopPropagation();
    const text = `Check out "${item.name}" at Ashoka Tiles!\n${window.location.href}`;
    if (navigator.share) {
      try { await navigator.share({ title: item.name, text, url: window.location.href }); } catch (_) {}
    } else {
...
function ImageViewer({ item, onClose, categoryName }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-white flex flex-col" role="dialog" aria-modal="true">
      ...
      <div className="flex-1 flex overflow-hidden">
        {images.length > 1 && (
          <div ref={thumbRef} className="hidden sm:flex flex-col gap-2 w-20 shrink-0 overflow-y-auto py-4 px-2 border-r border-line">
            {images.map((img, i) => (
              <button key={i} data-active={i === current ? "true" : "false"} onClick={() => setCurrent(i)}
                className={`shrink-0 w-full aspect-square rounded overflow-hidden border-2 transition-all ${i === current ? "border-accent shadow-sm scale-105" : "border-transparent opacity-60 hover:opacity-100 hover:border-stone/40"}`}>
        ...
        <div className="flex-1 flex items-center justify-center relative bg-white overflow-hidden" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
          <img key={current} src={images[current]} alt={`${item.name} - view ${current + 1}`}
            className="max-h-full max-w-full object-contain p-4 sm:p-8 transition-opacity duration-200"
            style={{ animation: "fadeIn 0.2s ease" }} />
          {images.length > 1 && (
            <>
              <button onClick={prev} className="absolute left-2 sm:left-4 w-9 h-9 rounded-full bg-white border border-line shadow-sm flex items-center justify-center text-ink hover:bg-bg transition-colors" aria-label="Previous image"><FiChevronLeft size={18} /></button>
              <button onClick={next} className="absolute right-2 sm:right-4 w-9 h-9 rounded-full bg-white border border-line shadow-sm flex items-center justify-center text-ink hover:bg-bg transition-colors" aria-label="Next image"><FiChevronRight size={18} /></button>
            </>
          )}
          {images.length > 1 && (
            <span className="absolute bottom-3 right-3 bg-ink/60 text-white text-[11px] px-2.5 py-1 rounded-full">{current + 1} / {images.length}</span>
          )}
        </div>
      </div>

      {images.length > 1 && (
        <div ref={thumbRef} className="sm:hidden shrink-0 flex gap-2 overflow-x-auto px-4 py-3 border-t border-line">
          {images.map((img, i) => (
            <button key={i} data-active={i === current ? "true" : "false"} onClick={() => setCurrent(i)}
              className={`shrink-0 w-14 h-14 rounded overflow-hidden border-2 transition-all ${i === current ? "border-accent scale-105" : "border-transparent opacity-55 hover:opacity-90"}`}>
      ...
      <div className="shrink-0 flex items-center justify-between px-4 sm:px-6 py-3 border-t border-line gap-3">
        <div className="min-w-0">
          <p className="font-display text-sm sm:text-base font-semibold text-ink truncate">{item.name}</p>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <a href={whatsappLink(`Hi, I'm interested in "${item.name}" from ${categoryName}. Please share details.`)}
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <main className="flex-1">
        ...
        <section className="py-12 sm:py-16 lg:py-20 max-w-7xl mx-auto px-4 sm:px-8">
          {category.items.length === 0 ? (
            <div className="text-center py-20">
              ...
              <a href={whatsappLink(`Hi, I'm interested in ${category.name}. Please share available options.`)}
                target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="relative aspect-square overflow-hidden w-full">
                      <button type="button" className="absolute inset-0 w-full h-full" onClick={() => setActiveItem(i)} aria-label={`View ${item.name}`}>
                        ...
                        <span className="absolute inset-0 bg-ink/0 group-hover:bg-ink/10 transition-colors flex items-center justify-center">
                          <span className="opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 text-ink text-xs font-semibold px-3 py-1.5 rounded-full">View Full</span>
                        </span>
                      </button>
                      <ShareOnCard item={item} categoryName={category.name} />
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      <h3 className="font-display text-base font-semibold text-ink">{item.name}</h3>
                      {item.description && (
                        <div className="mt-2 border-t border-line pt-2">
                          <p className="text-xs text-stone mt-1 leading-relaxed">{item.description}</p>
                        </div>
                      )}
                      <a href={whatsappLink(`Hi, I'm interested in "${item.name}" from ${category.name}. Please share details.`)}
                        target="_blank" rel="noopener noreferrer"
        ...
        {/* Other sanitaryware types */}
        <section className="bg-surface border-t border-line py-12 sm:py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-8">
            <h2 className="font-display text-xl sm:text-2xl font-semibold text-ink mb-6">Explore Other Types</h2>
            <div className="flex flex-wrap gap-3">
              {sanitarywareItems.filter((c) => c.slug !== slug).map((c) => (
                <Link key={c.slug} to={`/sanitaryware/${c.slug}`} className="inline-flex items-center gap-1.5 rounded-full border border-line px-4 py-2 text-sm font-medium text-ink hover:border-accent hover:text-accent transition-colors">
